#!/usr/bin/env python3
"""Cold-start and post-crash delta bias in the broadcast GA fitness, parsed from training.log.
...
def parse_traces(windows: list[dict]) -> dict[tuple, list[dict]]:
    """(scenario, seed) -> list of {round, gen, idx, hp, acc, fitness} in log order."""
                ...
                for w in windows:
                    if w["start"] <= ts <= w["end"]:
                        ...
                        if rec is None or rec["round"] != rnd:
                            # older runs log round 1 before telemetry is up, idx gets filled later
...
def infer_missing_round1(traces: dict) -> None:
    """Gen 0 is plain round-robin, so a missing idx is (round - 1) % 4."""
...
def delta_bias_table(traces: dict) -> list[str]:
    ...
    # the best delta usually lands early or right after a crash, and elitism
    # keeps re-broadcasting that HP even when its later deltas are poor
