#!/usr/bin/env python3
"""Per-scenario peak/final accuracy table from the run_matrix.py summary CSV. Works mid-sweep too.
...
def _ci95_halfwidth(values: List[float]) -> float | None:
    """Normal-approx CI, only a rough guide with 3-5 seeds per cell."""
    n = len(values)
    if n < 2:
        return None
