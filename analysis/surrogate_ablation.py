"""Surrogate on vs off (ga_surrogate_cifar vs ga_perclient_cifar, 5 seeds each).

Writes surrogate_ablation.png and surrogate_ablation_report.md next to this file.
"""
...
def load_matrix():
    ...
    for row in csv.DictReader(MATRIX.open()):
        ...
        if not (row.get("peak_eval_acc") and row.get("wall_seconds")):
            continue  # killed runs leave these blank
...
def detect_crashes(curve, threshold_pp=10.0):
    """(round, drop_pp) for every round where eval-acc fell by threshold_pp or more."""
    crashes = []
...
def main():
    ...
    # same seed means same data partition, so pairing by seed is valid
