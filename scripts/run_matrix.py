#!/usr/bin/env python3
"""Runs a (config x seed) sweep one run at a time, skipping pairs already ok in the summary CSV.
...
def _read_existing_summary(path: Path) -> set[tuple[str, int]]:
    """Only ok rows count as done, so failed pairs get retried on relaunch."""
    if not path.exists():
        return set()
...
def _latest_run_id_after(reference_epoch: float) -> Optional[str]:
    """The runner always creates a new run dir, so the newest one is the run just launched."""
...
def _run_one(config: Path, seed: int, federation: str, tag: Optional[str],
    ...
    # flwr run exits 0 even when the simulation never starts (flower-simulation
    # not on PATH), so returncode alone would mark empty runs as ok
