...
def append_shared_pool(path: str, new_entries: List[Dict]) -> None:
    """Append under an exclusive flock, since several clients write this file at once."""
    if not new_entries:
        return
...
class GeneticAlgorithm:
    def __init__(self, model: nn.Module, trainset: Dataset, testset: Dataset):
        ...
        # persisted between rounds by save_state/load_state
        self.population = []
        self.history = []
        self.elite = []
        ...
        # (hp_sig, rung) -> (round, payload), reused until FITNESS_CACHE_MAX_ROUND_AGE
    ...
    # ray recycles the client actors between rounds, so GA state has to live on disk
    ...
    def _sample_shared_indices(self, subset_ratio: float) -> np.ndarray:
        ...
        seed = config.SEED + (self.round_counter * 997)  # fixed per round, data is still per client
    ...
    def _crossover(self, ind1: creator.Individual, ind2: creator.Individual) -> Tuple[creator.Individual, creator.Individual]:
        """Swaps a single random gene; momentum follows the optimizer."""
    ...
    def _tournament_select(self, population: List[creator.Individual], k: int, tournsize: int) -> List[creator.Individual]:
        ...
        if len(valid_pop) < k:
            # happens after restoring a partial population, or when stagnation grows offspring_size
    ...
    def _cache_get(self, sig: Tuple, rung_tag: str) -> Optional[Tuple[float, float, float]]:
        ...
        if self.round_counter - cached_round > config.FITNESS_CACHE_MAX_ROUND_AGE:
            return None
        # old state files cached a bare fitness float; pad it to (fitness, drift, val_acc)
    ...
    def _history_entry(
        ...
        """post_agg_proxy is stored apart from fitness so the surrogate can train on either."""
    ...
    def _current_fitness_lambda(self) -> float:
        """Drift penalty with a linear warmup, since early drift mostly reflects an untrained global."""
    ...
    def _evaluate_rung(
        ...
        """(fitness, drift, val_acc), or None on CUDA OOM so no fake low score hits the surrogate."""
    ...
    def _compute_drift(self, global_state: Dict, device: torch.device) -> float:
        """RMS of (local - global) over all params, so it doesn't grow with model size."""
        with torch.no_grad():
            total_sq = 0.0
            total_n = 0
    ...
    def run_round_updates(self, global_state_dict: Dict, client_id: int) -> Dict:
        """One GA step per client visit. Returns (best_hp, fitness)."""
        ...
        if config.ENABLE_SURROGATE_GA:
            ...
            if surrogate_first:
                ...
                # predicted-only entries stay out of history, or the surrogate trains on its own output
            ...
            # elite is rebuilt from this round's scores only
