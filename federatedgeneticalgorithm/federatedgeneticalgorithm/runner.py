"""Runs `flwr run` with the config overrides from a YAML scenario file."""
...
APP_DIR = PACKAGE_DIR.parent  # flwr app dir, has pyproject.toml
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # keep the snapshot on failure so the run can be replayed
        if rc == 0:
            snapshot_path.unlink(missing_ok=True)
        else:
