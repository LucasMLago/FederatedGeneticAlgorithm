...
def _pid_alive(pid: int) -> bool:
    ...
    try:
        ...
        # another user's process, count it as alive so their run dir is left alone
        return True
    return True


def _parse_active_run(raw: str) -> Tuple[Optional[int], str]:
    """Parses "<pid>:<run_id>", or just "<run_id>" from older runs."""
    stripped = raw.strip()
    if ":" in stripped:
...
def _archive_runner_inputs(run_dir: Path) -> None:
    # the runner deletes its JSON tempfile after a clean exit, so keep copies here
...
def _base_dir() -> Path:
    # must be absolute, Ray workers run with cwd under /tmp/ray and the CSVs would land there
    ...
    app_root = Path(__file__).resolve().parents[1]  # flwr app dir, has pyproject.toml
...
def initialize_run(force_new: bool = False, record_config: bool = False) -> str:
    """Create or reuse the run_id shared by server and clients."""
    ...
    # dead writer pid = leftover from a crashed run. Ray workers have their own pid,
    # but the server that wrote the file is still alive, so they reuse its run_id
    must_create = (
        force_new
