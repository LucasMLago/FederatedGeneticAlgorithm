...
def _get_local_datasets(partition_id: int, num_partitions: int):
    ...
    # IID test split so eval reflects the global model, not this client's skew
...
def _ga_state_path(partition_id: int) -> str:
    # GA state lives on disk because Ray can recycle the actor between rounds
    return str(telemetry.get_run_dir() / "ga_state" / f"partition_{partition_id}.pkl")


def _extract_fed_ga_hp(msg: Message):
    """HP broadcast by the server-side search, or None."""
    try:
...
def train(msg: Message, context: Context):
    ...
    # server-side HP search wins over the local GA
    ...
    if fed_ga_hp is not None:
        ...
        # always reload, another actor may have run this partition last round
