"""Server-side GA: every client gets the same HP each round, so FedAvg never mixes HPs."""
...
@dataclass
class FederatedGA:
    """One HP per FL round; its fitness is the eval acc of the aggregated model."""
    ...
    def select_for_round(self, server_round: int) -> Dict:
        """Round-robin through the current generation."""
        return dict(self.population[self.current_idx])

    def record_fitness(self, fitness: float) -> Dict[str, object]:
        """Score the HP just used; evolves once the whole generation has a score."""
